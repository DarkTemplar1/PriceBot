...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        ttk.Label(row_ctrl1, text="obniżka % ceny:").pack(side="left", padx=(8, 2))
        ttk.Spinbox(row_ctrl1, from_=0.0, to=100.0, increment=0.5,
                    width=6, textvariable=self.margin_pct_var).pack(side="left")
        ttk.Label(row_ctrl1, text="± m²:").pack(side="left", padx=(12, 2))
    ...
    def automate(self):
        """
        Uruchamia Automat.py/automat.py w tle, przekazując:
          --raport <wybrany plik raportu>
          --oferty <Polska.xlsx z folderu bazowego>
        Zmienia kolor przycisku (żółty → pracuje, zielony → zakończone)
        i pokazuje komunikat po zakończeniu.
        """

        raport_path = self.input_file_var.get().strip()
        if not raport_path:
            messagebox.showerror("Automat", "Najpierw wybierz plik raportu (u góry).")
            return

        base_dir = Path(self.folder_var.get()).resolve()
        oferty_path = base_dir / "Polska.xlsx"
        if not oferty_path.exists():
            messagebox.showerror(
                "Automat",
                f"Nie znaleziono pliku z ofertami:\n{oferty_path}\n"
                "Upewnij się, że Polska.xlsx jest w folderze bazowym."
            )
            return
        ...
        def worker():
            ...
            if script_path is None:
                def no_script():
                    messagebox.showerror("Automat", "Nie znaleziono Automat.py ani automat.py w folderze aplikacji.")
            ...
            try:
                # przekazujemy ścieżki jako parametry CLI zgodnie z automat.py
                cmd = [
                    sys.executable,
                    str(script_path),
                    "--raport", raport_path,
                    "--oferty", str(oferty_path),
                ]
            ...
            def on_done():
                ...
                try:
                    ...
                    pass
                messagebox.showinfo("Automat", "Zakończono działanie Automat.py")
